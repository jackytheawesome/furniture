...
- Смета: у каждого блока предмета в шапке — **редактируемые параметры** из корзины (PATCH item → regenerate).
- Смета: у каждого блока **«+ Статья»** — из прайса (поиск) или произвольно; строка с `cartItemId`.
- Строки `category === "board"` / `"facade"`: **PriceCombobox** из прайса; выбор → `boardMaterialKey` / `hdfMaterialKey` / `facadeMaterialKey` + regenerate.
- Числовые поля: `NumberField` — можно стереть значение без залипания ведущего `0` (`030`).
- Корзина: список предметов + импорт файлов; параметры также правятся в шапке блока сметы.
...
9. Combobox плиты и фасадов есть; кромка/фурнитура — пока нет.
10. ~~Редактирование params предмета из корзины — не сделано.~~ Редактируется в шапке блока сметы.
